...
/**
 * 存储词和词性的数据结构，高效存储，快速搜索
 * 使用前缀树来实现
 * 为前缀树的一级节点建立索引（比二分查找要快）
 * @author 杨尚川
 */
public class PartOfSpeechTrie {
    private static final Logger LOGGER = LoggerFactory.getLogger(PartOfSpeechTrie.class);
    ...
    public String get(String item, int start, int length){
        if(start < 0 || length < 1){
            return null;
        }
    ...
    public void put(String item, String pos){
        //去掉首尾空白字符
        item=item.trim();
        int len = item.length();
        if(len < 1){
    ...
    private void show(TrieNode node, String indent){
        if(node.isTerminal()){
            LOGGER.info(indent+node.getCharacter()+"="+node.getPos()+"(T)");
        }else{
    ...
    public static void main(String[] args){
        PartOfSpeechTrie trie = new PartOfSpeechTrie();
        trie.put("杨尚川", "nr");
        trie.put("杨尚喜", "nr");
        trie.put("人", "n");
        trie.put("写代码", "v");
